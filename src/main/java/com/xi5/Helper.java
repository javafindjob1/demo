...
public class Helper {
  ...
  public static void handleJ(List<Function> funList, Map<String, UnitDetail> idUnitMap,
    ...
    try (
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
            new FileOutputStream("D:\\Code\\demo\\demo\\src\\main\\java\\com\\xi5\\custom\\hero0.ini"), "utf8"))) {
  ...
  private static Pattern ultiPat = Pattern.compile(
      "GetUnitTypeId\\(GetTriggerUnit\\(\\)\\)=='(\\w{4})'( or GetUnitTypeId\\(GetTriggerUnit\\(\\)\\)=='(\\w{4})')? and GetUnitAbilityLevel\\(GetTriggerUnit\\(\\),'(\\w{4})'\\)==0 and GetUnitLevel\\(GetTriggerUnit\\(\\)\\)>=15");
  ...
  public static List<String> handleUlti(List<Function> funList, Map<String, UnitDetail> idUnitMap) {
    ...
    for (Function fun : funList) {
      ...
      for (String row : rows) {
        if (row.contains(")==0 and GetUnitLevel(GetTriggerUnit())>=15")) {
