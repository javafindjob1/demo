...
import com.xi3.parse.ItemDetail.UnitDrop;
...
public class ItemParse extends AbstractParse {
  ...
  public Map<String, ItemDetail> parse(List<Item> items) {
    ...
    // 处理制作卷轴和炼化书
    nameMap.keySet().stream().filter(name -> name.contains("制作书") ||name.contains("合成卷轴") ||  name.contains("锻造书") || name.contains("炼化书") || name.contains("制作卷轴")).forEach(name -> {
      Pattern p = Pattern.compile("【(.+)】", Pattern.CASE_INSENSITIVE);
      Matcher matcher = p.matcher(name);
      if(!matcher.find()) {
        throw new RuntimeException("解析物品卷轴失败:" + name);
      }
  ...
  private static String splitSynthesisFormula(String description) {
    ...
    String str = split("需要材料|n", "|n|cFF99CC00", description);
    return "|r" + str;
  }
  ...
  public static String splitName(String desc) {
    ...
    return desc.replaceAll("(?i)\\|cff\\w{3,6}", "").replaceAll("(\\|r)|(\\|n)", "").replace(".", ".");
  }
