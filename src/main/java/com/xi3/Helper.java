...
public class Helper {
  ...
  public static void main(String[] args) throws Exception {
    ...
    /*
         E006 "A016,A015,A15J,A013"
觉醒 E02T "A016,A015,A15J,A013"

     E04D "A016,A015,A15J,A013"
觉醒 E04E "A016,A015,A15J,A013"
     */
    idUnitMap.get("E02T").setName("银之月姬觉醒");
    idUnitMap.get("E04E").setName("银之月姬觉醒");
  ...
  public static void handleJ(List<Function> funList, Map<String, UnitDetail> idUnitMap,
    ...
    try (
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(
            new FileOutputStream("D:\\Code\\demo\\demo\\src\\main\\java\\com\\xi3\\custom\\hero0.ini"), "utf8"))) {
  ...
  private static Pattern ultiPat = Pattern.compile(
   ...
   // 大招相同的英雄ID
  static Map<String,String> ultiMap = new HashMap<>();
  public static List<String> handleUlti(List<Function> funList, Map<String, UnitDetail> idUnitMap) {
    ...
    for (Function fun : funList) {
      ...
      for (String row : rows) {
        if (row.contains("and GetUnitLevel(GetTriggerUnit())>=15 and GetUnitAbilityLevel")) {
      ...
      for (int i = 0; i < rows.size(); i++) {
        ...
        if (row.contains("call DisplayTimedTextToPlayer(GetTriggerPlayer(),0,0,10.,\"|cffffcc00")) {
          ...
          if (preRow.contains(" GetUnitTypeId(GetTriggerUnit())=='") && matcher.find()) {
            ...
            String id2 = team[1];
            if (id2!= null) {
              UnitDetail u1 = idUnitMap.get(id1);
              UnitDetail u2 = idUnitMap.get(id2);
              // 这是个皮肤，可以添加增强内容
              String heroName = u1.getName();
              for (AbilityDetail abi : abilityMap.values()) {
                String abiZengqiang = abi.getName();
                if (abiZengqiang!=null && abiZengqiang.contains(heroName) && abiZengqiang.contains("的皮肤")) {
                  introList.add(u2.getId() + " = \"" + abi.getUbertip() + "\"");
                }
              }
            }
          } else {
