...
public class UnitSheet extends AbstractSheet {
    ...
    public void insertHead(String sheetName, Map<String, Hero[]> heroMap) throws IOException {
        ...
        int dataI = -1;
        {
            ...
            short height = (short) (sheet.getDefaultRowHeight() * 3);
            {
                ...
                XSSFRichTextString richText = createRichTextString((short) 30, "#FF0000", "此文档特别鸣谢：");
...

            {
                ...
                sheet.addMergedRegion(new CellRangeAddress(dataI, dataI, 0, LAST_COL)); // 行从0开始，列从0开始
                Cell cell = row.createCell(0);
                XSSFRichTextString richText = createRichTextString((short) 30, "#FF0000", "高音王子（装备剪辑，贴图，文本介绍）");
                cell.setCellValue(richText);
            }

            {
                ...
                sheet.addMergedRegion(new CellRangeAddress(dataI, dataI, 0, LAST_COL)); // 行从0开始，列从0开始
                Cell cell = row.createCell(0);
                XSSFRichTextString richText = createRichTextString((short) 30, "#FF0000", "玉露、TFBOYS-王俊凯（备注，点评）");
                cell.setCellValue(richText);
            }
            {
                Row row = sheet.createRow(++dataI);
                ...
                sheet.addMergedRegion(new CellRangeAddress(dataI, dataI, 0, LAST_COL)); // 行从0开始，列从0开始
                Cell cell = row.createCell(0);
                XSSFRichTextString richText = createRichTextString((short) 30, "#FF0000", "热心的梦群群友");
                cell.setCellValue(richText);
            }
        }

        {
            ...
            // 设置单元格值为超链接
            XSSFRichTextString richText = createRichTextString((short) 10, "#000000", "【腾讯文档】梦想远景装备介绍_目录");
            cell.setCellValue(richText);
        }
        {
            // 创建一个新的行
            ...
            // 设置单元格内容
            XSSFRichTextString richText = createRichTextString((short) 10, "#0000FF", XSSFFont.U_SINGLE, "返回目录");
            cell.setCellValue(richText);
        }

        {
            ...
            XSSFRichTextString richText = createRichTextString((short) 14, "#FF0000", "近期更新记录（具体内容在下方标签页查看）：");
            cell.setCellValue(richText);

            try (SqLiteJDBC db = new SqLiteJDBC()) {
                // 连接SQLite数据库，数据库文件是test.db，如果文件不存在，会自动创建
                AtomicInteger dataIn = new AtomicInteger(14);
...

        {
            ...
            for (int i = 0; i < heroArr.length; i++) {
                ...
                for (int j = 0; j < heroArr[0].length; j++) {
                    ...
                    columnStart += 2;
                    sheet.addMergedRegion(new CellRangeAddress(rowStart, rowStart, columnStart - 2, columnStart)); // 行从0开始，列从0开始
                    ...
                    buf.append(UnitParse.trimName(u.getPropernames())).append("（").append(u.getName()).append("）");
                    XSSFRichTextString richText = createRichTextString((short) 10, "#000000", buf.toString());
                    cell.setCellValue(richText);
    ...
    private void updateInfo(XSSFSheet sheet, AtomicInteger dataI, String message, List<ItemDetail> list) {
        ...
        for (ItemDetail item : list) {
            ...
            if (!item.getType().equals(itemType)) {
                ...
                Cell cell = row.createCell(1);
                XSSFRichTextString richText = createRichTextString((short) 12, "#b65656", itemType);
                cell.setCellValue(richText);
            }
            // 新增S+武器(龙翼)
            Row row = sheet.createRow(dataI.incrementAndGet());
            sheet.addMergedRegion(new CellRangeAddress(dataI.get(), dataI.get(), 1, 8)); // 行从0开始，列从0开始
            ...
            XSSFRichTextString richText = createRichTextString((short) 11, "#000000", buf.toString());
            cell.setCellValue(richText);
        }
    }
    ...
    public void insert(String sheetName, Map<String, List<UnitDetail>> mapList) throws IOException {
...

        {
            ...
            {
                Cell cell = row.createCell(1);
                XSSFRichTextString richText = createRichTextString((short) 13, "#000000", " ");
                appendRichText(richText, (short) 13, "#FF0000", " ");
                cell.setCellValue(richText);
            }
        }
        ...
        for (Entry<String, List<UnitDetail>> e : mapList.entrySet()) {
            {
                ...
                XSSFRichTextString richText = createRichTextString((short) 18, "#999999", key);
                cell.setCellValue(richText);
