...
public class ItemParse extends AbstractParse {
  ...
  private static String splitLevel(String description) {
    ...
    String[] infs = info.split("·");
    if(infs.length>1 && infs[1].equals("辉")){
      info = infs[0] + "·" + infs[1];
    }else{
      info = infs[0];
    }
  ...
  public void wrapDropString(Map<String, UnitDetail> idUnitMap, Map<String, DestructableDetail> destructableMap,
      Map<String, FunctionDetail> funDetailList) {
    ...
    idUnitMap.forEach((unitId, unitDetail) -> {
      ...
      for (String itemId : items) {
        List<ItemDetail.UnitDrop> list2 = MapUtil.getNotNull(shopSellItemMap, itemId, ArrayList::new);
    ...
    funDetailList.values().stream().filter(e -> e.getWaJueList().size() > 0).map(e -> e.getWaJueList())
        .forEach(dropList -> {
          ...
          dropList.forEach((item) -> {
            List<ItemDetail.UnitDrop> list2 = MapUtil.getNotNull(monsterDropItemMap, item.getItemId(),
                ArrayList::new);
