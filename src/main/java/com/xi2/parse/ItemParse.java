...
import com.xi2.Client;
import com.xi2.parse.ItemDetail.UnitDrop;
...
public class ItemParse extends AbstractParse {
  ...
  public Map<String, ItemDetail> parse(List<Item> items, Map<String, AbilityDetail> abilityDetailMap) {

    parse(items);

    AbilityDetail a0I2 = abilityDetailMap.get("A0I2");
    if(a0I2!=null&&a0I2.getUbertips()!=null){
      String[] ubertips = a0I2.getUbertips();
      for (int i = 1; i < ubertips.length; i++) {
        String newId = "A0I2_"+(i+1);
        ItemDetail itemDetail = new ItemDetail(a0I2.getName());
        itemDetail.setId(newId);
        itemDetail.setIcon(a0I2.getArt());
        itemDetail.setDescription(ubertips[i]);
        itemMap.put(newId, itemDetail);
      }
    }
    return itemMap;
  }
  public Map<String, ItemDetail> parse(List<Item> items) {
    ...
    // 处理制作卷轴和炼化书
    nameMap.keySet().stream().filter(name -> name.contains("制作书") || name.contains("锻造书") || name.contains("炼化书") || name.contains("制作卷轴")).forEach(name -> {
      ...
      case "套装":
        ...
        if (zhixianMap != null) {
          ...
          if (!itemListMap.isEmpty()) {
            ...
            otherList.addAll(itemListMap.values());
            sortList(otherList);
          }
        } else {
          sortList(itemList);
  ...
  private static String splitSynthesisFormula(String description) {
    // 需要材料|n|cff00ff00莲华神杖|r|n|cff00ff00洗礼圣瓶|r|n|cff00ff00魔力碎片x5|r|n|n|cffff99cc
    String str = split("需要材料|n", "|n|cffff99cc", description);
    return "|r" + str;
  }
  ...
  public static String splitName(String desc) {
    ...
    return desc.replaceAll("(?i)\\|c\\w{8}", "").replaceAll("(?i)\\|[rn]", "").replace(".", ".");
  }
